...
const routes = [
  ...
  '/store/shop',
  '/store/categories',
  '/store/category/swimming',
  '/store/product/elite-hydro-pro-goggles',
  '/store/product/pitch-dominance-training-jersey',
  '/store/search?q=swim',
  '/store/cart',
  '/store/checkout',
  '/store/wishlist',
...
async function clickOutsideOverlay(page, overlay) {
  ...
  for (const point of candidates) {
    const outside =
      point.x < overlayBox.x ||
      point.x > overlayBox.x + overlayBox.width ||
      point.y < overlayBox.y ||
      point.y > overlayBox.y + overlayBox.height;
    if (!outside) continue;
      ...
      ({ x, y }) =>
        Boolean(
          document
            ...
            ?.closest('a,button,input,select,textarea,[role="button"]'),
        ),
      point,
    );
...
async function modalChecks(page, selector, trigger, close) {
  ...
  assert(await panel.evaluate((element) => element.contains(document.activeElement)), `${selector}: missing initial focus`);
  for (let i = 0; i < 16; i++) await page.keyboard.press('Tab');
  assert(await panel.evaluate((element) => element.contains(document.activeElement)), `${selector}: escaped focus trap`);
  await page.keyboard.press('Escape');
  await panel.waitFor({ state: 'hidden' });
  assert(await trigger.evaluate((element) => element === document.activeElement), `${selector}: failed focus restoration`);
  await trigger.click();
  const backdropClicked = await clickExposedBackdrop(page, close, panel);
  if (!backdropClicked) await page.keyboard.press('Escape');
  await panel.waitFor({ state: 'hidden' });
  if (!backdropClicked) {
    assert(await trigger.evaluate((element) => element === document.activeElement), `${selector}: failed fallback focus restoration`);
  }
...
async function interactions(browser, name, rtl) {
  ...
  try {
    ...
    await modalChecks(
      page,
      '.store-filter-layer .store-filters',
      page.locator('.store-mobile-filter'),
      page.locator('.store-filter-layer .store-drawer-backdrop'),
    );
    ...
    if (preview) assert.equal(await page.locator('.store-product-card').count(), 4, 'Swimming preview count drifted');
    ...
    assert(
      await page.locator('#store-search-results').evaluate((element) => element.contains(document.activeElement)),
      'search keyboard navigation failed',
    );
    await page.keyboard.press('Escape');
    assert(await search.evaluate((element) => element === document.activeElement), 'search focus restoration failed');
    ...
    await modalChecks(
      page,
      '.store-mini-cart',
      page.locator('.store-utilities>button[aria-label="Cart | السلة"]'),
      page.locator('.store-drawer-layer .store-drawer-backdrop'),
    );

    if (preview) {
      await visit(page, errors, '/store/shop');

      const card = page.locator('.store-product-card').filter({
        has: page.locator('h3', { hasText: 'Pitch Dominance Training Jersey' }),
      });
      await card.waitFor();
      ...
      await modalChecks(
        page,
        '.store-lightbox',
        page.locator('.store-gallery-expand'),
        page.locator('.store-lightbox-layer .store-drawer-backdrop'),
      );

      await page.locator('[role=tab]').filter({ hasText: rtl ? 'دليل المقاسات' : 'Size guide' }).click();
      const sizePanel = await page.locator('[role=tabpanel]').textContent();
      assert(
        sizePanel?.includes(rtl ? 'بيانات المقاسات غير مهيأة بعد' : 'Sizing information not configured yet'),
        'size-guide pending state missing',
      );

      const mobileBuy = page.locator('.store-mobile-buy button');
      assert.equal(await mobileBuy.isDisabled(), true, 'detail purchase should require color selection');
      await page.locator('.store-product-info').getByRole('button', { name: 'Black / Gold | أسود / ذهبي', exact: true }).click();
      assert.equal(await mobileBuy.isDisabled(), false, 'detail purchase did not unlock after valid selection');
      ...
      for (const input of await page.locator('.store-checkout-form input[required]').all()) {
        if (!(await input.inputValue())) await input.fill('QA');
      }
      await page.locator('.store-checkout-form button[type=submit]').click();
          ...
          try {
            for (const route of routes) {
              ...
              if (
                ...
                ['/store', '/store/shop', '/store/product/elite-hydro-pro-goggles'].includes(route)
              ) {
                await screenshot(
                  page,
                  `${preview ? 'preview' : 'production'}-${width}-${theme}-${rtl ? 'rtl' : 'ltr'}-${route.split('/').at(-1)}`,
                );
              }
              count++;
            }
...
for (const [name, type] of Object.entries({ Chromium: chromium, Firefox: firefox, WebKit: webkit })) {
  if (!selected || selected === name) await matrix(type, name);
}
