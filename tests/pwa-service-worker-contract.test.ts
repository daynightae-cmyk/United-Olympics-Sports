...
assert.match(sw, /if \(isNetworkOnlyStaticPath\(url\.pathname\)\) return;/, 'network-only portal artwork must bypass respondWith');
...
assert.match(
  ...
  'cached static asset requests must settle deterministically on network failure',
);
assert.match(sw, /event\.respondWith\(serveStaticAsset\(request\)\);/, 'cacheable static assets must use the resilient handler');
assert.equal(
