    ...
    <div className="sport-media-manager admin-panel">
      <div className="sport-media-heading">
        <div>
          ...
          <h2>
            <BilingualText
              value={bi("Verified user media collection", "مجموعة وسائط معتمدة من المستخدم")}
            />
          </h2>
          <p>
            <BilingualText
              value={bi(
                "Read-only Phase 1 media manager. Public sport pages and Admin use the same typed source of truth, with local rendering paths and original provenance kept together.",
                "مدير وسائط للقراءة فقط في المرحلة الأولى. تستخدم صفحات الرياضات العامة والإدارة نفس المصدر الموحّد، مع حفظ مسار العرض المحلي ومرجع المصدر الأصلي معًا."
              )}
            />
          </p>
        </div>
      ...
      >
        <div className="sport-media-row sport-media-header" role="row">
          <strong>
            <BilingualText value={bi("Image Preview", "معاينة الصورة")} />
          </strong>
          <strong>
            <BilingualText value={bi("Usage", "الاستخدام")} />
          </strong>
          <strong>
            <BilingualText value={bi("Order", "الترتيب")} />
          </strong>
          <strong>
            <BilingualText value={bi("Alt Text", "النص البديل")} />
          </strong>
          <strong>
            <BilingualText value={bi("Source & Status", "المصدر والحالة")} />
          </strong>
        </div>
        {ordered.map((asset) => {
              ...
              <div className="sport-media-source" role="cell">
                ...
                <span className={isLocal ? "source-chip local" : "source-chip remote"}>
                  <BilingualText
                    value={
                      isLocal
                        ? bi("Local Asset", "أصل محلي")
                        : bi("Remote Rendering Source", "مصدر عرض خارجي")
                    }
                  />
                </span>
                {asset.sourceUrl && (
