...
const sections: Array<{ title: { en: string; ar: string }; items: NavItem[] }> = [
  ...
  {
    title: bi("Training Operations", "عمليات التدريب"),
    items: [
      {
        to: "/admin/groups",
        label: bi("Training Groups / Teams", "الفرق / مجموعات التدريب"),
        icon: "groups",
      },
  ...
  {
    title: bi("Finance", "المالية"),
    items: [
      {
        to: "/admin/subscriptions",
        label: bi("Subscriptions", "الاشتراكات"),
        icon: "subscriptions",
      },
  ...
  {
    title: bi("Communications", "التواصل"),
    items: [
      {
        to: "/admin/announcements",
        label: bi("Announcements", "الإعلانات"),
        icon: "announcements",
      },
  ...
  {
    title: bi("Insights & Governance", "الرؤى والحوكمة"),
    items: [
  ...
  {
    title: bi("Experience & Access", "التجربة والوصول"),
    items: [
...
export function AdminSidebar({ open, collapsed, onClose, onCollapse }: Props) {
  return (
    <aside
      ...
      aria-label="Admin navigation | تنقل الإدارة"
    >
      ...
      <nav className="admin-nav">
        {sections.map((section) => (
          <section className="admin-nav-section" key={section.title.en}>
            <BilingualText value={section.title} className="admin-nav-heading" />
            {section.items.map((item) => {
              const Icon = adminIconRegistry[item.icon];
              return (
                <NavLink
                  key={item.to}
                  to={item.to}
                  ...
                  onClick={onClose}
                  title={`${item.label.en} | ${item.label.ar}`}
                >
      ...
      >
        ...
        <BilingualText value={collapsed ? bi("Expand", "توسيع") : bi("Collapse", "طي القائمة")} />
      </button>
    </aside>
  );
}
