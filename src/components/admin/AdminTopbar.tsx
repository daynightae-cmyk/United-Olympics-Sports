...
import { PortalUtilityNav } from '../navigation/PortalUtilityNav';

type Props = { title: { en: string; ar: string }; onMenu: () => void; menuButtonRef: RefObject<HTMLButtonElement | null>; sidebarOpen: boolean };
const quickLinks = [
...
export function AdminTopbar({ title, onMenu, menuButtonRef, sidebarOpen }: Props) {
  ...
  return <header className="admin-topbar">
    <div className="admin-topbar-title"><button ref={menuButtonRef} className="admin-icon-button mobile-only" onClick={onMenu} aria-label="Open navigation | فتح القائمة" aria-expanded={sidebarOpen} aria-controls="admin-portal-navigation"><Menu /></button><div><small><BilingualText value={bi('Current Page', 'الصفحة الحالية')} /></small><BilingualText value={title} /></div></div>
    <PortalUtilityNav homeTo="/admin" compact />
