...
export function PlayerAthleteIdentityCard({
  ...
  const nextSessionText = nextSession
    ? formatPlayerDateTime(nextSession.startsAt, bilingualOrder)
    : undefined;

  return (
    <section
      className={`cgpt-athlete-id ${sportGeometryClass(sport?.id)}`}
      aria-labelledby="cgpt-athlete-name"
    >
      ...
      <div className="cgpt-athlete-id__portrait-zone">
        ...
        <div className="cgpt-athlete-id__sport-stage" aria-hidden>
          <Sports3DStage sportKey={sport?.id ?? null} size={220} opacity={0.14} />
        </div>
        <div className="cgpt-athlete-id__monogram" aria-hidden="true">
          UOS
        </div>
      </div>
      ...
      <div className="cgpt-athlete-id__content">
        <div className="cgpt-athlete-id__eyebrow">
          <span className="cgpt-athlete-id__sport">
            <BilingualText value={sport?.name ?? bi("Athlete", "لاعب")} />
          </span>
          {preview && (
            <span className="cgpt-athlete-id__preview">
              <Sparkles size={12} />
        ...
        <div className="cgpt-athlete-id__context">
          ...
          {coach && (
            <span>
              <Medal size={14} />
              {bilingualOrder === "ar-first" ? coach.nameAr : coach.nameEn}
            </span>
          )}
        </div>
