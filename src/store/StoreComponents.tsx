...
export function StorePreviewNotice() {
  ...
  return <div className="store-truth-notice" role="status"><Sparkles aria-hidden="true" /><StoreCopy value={{ en: 'Development preview · catalog values are not live inventory', ar: 'معاينة تطوير · قيم الكتالوج ليست مخزونًا فعليًا' }} inline /></div>;
}
    ...
    <div className="store-main-header store-reference-mainbar">
      ...
      <div className="store-search-wrap store-reference-search-wrap" ref={searchWrapRef} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setSearchOpen(false); }}>
        <form className="store-search store-reference-search" role="search" onSubmit={(event) => { event.preventDefault(); submitSearch(); }}>
          <label className="sr-only" htmlFor="store-search">Search products | البحث عن المنتجات</label>
          <input ref={searchRef} aria-expanded={searchOpen && query.trim().length > 1} aria-controls="store-search-results" onKeyDown={(event) => { if (event.key === 'ArrowDown') { event.preventDefault(); searchWrapRef.current?.querySelector<HTMLAnchorElement>('.store-search-results a')?.focus(); } }} id="store-search" value={query} onFocus={() => setSearchOpen(true)} onChange={(event) => { setQuery(event.target.value); setSearchOpen(true); }} placeholder={locale === 'ar' ? 'ابحث عن المنتجات، الرياضات، الفئات…' : 'Search for products, categories…'} />
          <button type="submit" aria-label="Submit search | تنفيذ البحث"><Search aria-hidden="true" /></button>
        </form>
      ...
      <div className="store-utilities store-reference-utilities">
        <button className="store-language" type="button" aria-label="Change language | تغيير اللغة" onClick={() => setLocale(locale === 'en' ? 'ar' : 'en')}>{locale === 'en' ? 'ع' : 'EN'}</button>
        <ThemeToggle compact />
        <Link to="/store/account" aria-label="Account | الحساب"><UserRound /><span><StoreCopy value={{ en: 'Account', ar: 'الحساب' }} /></span></Link>
        <Link to="/store/wishlist" aria-label="Wishlist | المفضلة"><Heart /><i>{wishlist.length}</i><span><StoreCopy value={{ en: 'Wishlist', ar: 'المفضلة' }} /></span></Link>
        <button type="button" onClick={() => setMiniCartOpen(true)} aria-label="Cart | السلة"><ShoppingBag /><i>{cartCount}</i><span><StoreCopy value={{ en: 'Cart', ar: 'السلة' }} /></span></button>
      </div>
    </div>

    <nav className={`store-nav store-reference-nav ${menuOpen ? 'is-open' : ''}`} aria-label="Store navigation | تنقل المتجر">
      <button type="button" ref={megaRef} className="store-mega-trigger" onClick={() => setMegaOpen((open) => !open)} aria-expanded={megaOpen}><Menu /><StoreCopy value={{ en: 'SHOP BY CATEGORY', ar: 'تسوق حسب الفئة' }} inline /><ChevronDown /></button>
      <NavLink to="/store/shop?collection=new" onClick={() => setMenuOpen(false)}><StoreCopy value={{ en: 'New Arrivals', ar: 'وصل حديثًا' }} /></NavLink>
      <NavLink to="/store/category/apparel" onClick={() => setMenuOpen(false)}><StoreCopy value={{ en: 'Men', ar: 'رجال' }} /></NavLink>
      <NavLink to="/store/category/apparel" onClick={() => setMenuOpen(false)}><StoreCopy value={{ en: 'Women', ar: 'نساء' }} /></NavLink>
      <NavLink to="/store/category/football" onClick={() => setMenuOpen(false)}><StoreCopy value={{ en: 'Football', ar: 'كرة القدم' }} /></NavLink>
      <NavLink to="/store/category/swimming" onClick={() => setMenuOpen(false)}><StoreCopy value={{ en: 'Swimming', ar: 'السباحة' }} /></NavLink>
      <NavLink to="/store/category/equipment" onClick={() => setMenuOpen(false)}><StoreCopy value={{ en: 'Equipment', ar: 'المعدات' }} /></NavLink>
