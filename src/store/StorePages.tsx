...
export function CheckoutPage() {
  const { cart, clearCart, isPreview } = useStore();
  ...
  const next = (event: FormEvent<HTMLFormElement>) => {
    ...
    void (async () => {
      ...
      try {
        ...
        if (!token) { setOrder({ status: 'auth-required' }); return; }
        const res = await fetch('/api/v1/store/checkout', {
          ...
          body: JSON.stringify({
            ...
            shippingAddress: {
              email: snapshot.email, phone: snapshot.phone, fullName: snapshot.fullName,
              country: snapshot.country, emirate: snapshot.emirate, city: snapshot.city,
              area: snapshot.area, street: snapshot.street, building: snapshot.building,
              apartment: snapshot.apartment, instructions: snapshot.instructions,
            },
          }),
        });
        const payload = await res.json().catch(() => null) as { ok?: boolean; order?: { orderNumber?: string }; error?: { message?: string } } | null;
        if (!res.ok || !payload?.ok) throw new Error(payload?.error?.message || `HTTP_${res.status}`);
        setOrder({ status: 'placed', orderNumber: payload.order?.orderNumber ?? '' });
        clearCart();
      } catch (e) {
        setOrder({ status: 'failed', message: e instanceof Error ? e.message : 'ORDER_FAILED' });
      }
    })();
  };
  return <div className="store-checkout-page"><div className="store-page-heading"><span>{isPreview ? 'CHECKOUT PREVIEW' : 'SECURE CHECKOUT'}</span><h1><StoreCopy value={{ en: 'Checkout', ar: 'إتمام الطلب' }} /></h1></div><div className="store-checkout-stepper">{checkoutSteps.map(({ id, icon: Icon, value }) => <button type="button" key={id} className={`${step === id ? 'is-active' : ''} ${step > id ? 'is-complete' : ''}`} onClick={() => id <= step && setStep(id)}><span>{step > id ? <CheckCircle2 /> : <Icon />}</span><i>{id}</i><StoreCopy value={value} /></button>)}</div><div className="store-checkout-layout"><form className="store-checkout-form" onSubmit={next}><header><span>0{step}</span><h2><StoreCopy value={checkoutSteps[step - 1].value} /></h2></header>{step === 1 && <div className="store-form-grid"><label><StoreCopy value={{ en: 'Email', ar: 'البريد الإلكتروني' }} inline /><input name="email" type="email" autoComplete="email" required placeholder="name@example.com" defaultValue={formSnapshot.email ?? ''} /></label><label><StoreCopy value={{ en: 'Phone', ar: 'رقم الهاتف' }} inline /><input name="phone" type="tel" autoComplete="tel" required placeholder="+971" defaultValue={formSnapshot.phone ?? ''} /></label></div>}{step === 2 && <div className="store-form-grid"><label className="wide"><StoreCopy value={{ en: 'Full Name', ar: 'الاسم الكامل' }} inline /><input name="fullName" autoComplete="name" required defaultValue={formSnapshot.fullName ?? ''} /></label><label><StoreCopy value={{ en: 'Country', ar: 'الدولة' }} inline /><select name="country" required defaultValue="AE"><option value="AE">United Arab Emirates | الإمارات</option></select></label><label><StoreCopy value={{ en: 'Emirate', ar: 'الإمارة' }} inline /><select name="emirate" required defaultValue={formSnapshot.emirate ?? ''}><option value="" disabled>Select | اختر</option><option>Abu Dhabi | أبوظبي</option><option>Dubai | دبي</option><option>Sharjah | الشارقة</option><option>Ajman | عجمان</option><option>Umm Al Quwain | أم القيوين</option><option>Ras Al Khaimah | رأس الخيمة</option><option>Fujairah | الفجيرة</option></select></label><label><StoreCopy value={{ en: 'City', ar: 'المدينة' }} inline /><input name="city" required defaultValue={formSnapshot.city ?? ''} /></label><label><StoreCopy value={{ en: 'Area', ar: 'المنطقة' }} inline /><input name="area" required defaultValue={formSnapshot.area ?? ''} /></label><label className="wide"><StoreCopy value={{ en: 'Street', ar: 'الشارع' }} inline /><input name="street" required defaultValue={formSnapshot.street ?? ''} /></label><label><StoreCopy value={{ en: 'Building', ar: 'المبنى' }} inline /><input name="building" required defaultValue={formSnapshot.building ?? ''} /></label><label><StoreCopy value={{ en: 'Apartment / Villa', ar: 'شقة / فيلا' }} inline /><input name="apartment" defaultValue={formSnapshot.apartment ?? ''} /></label><label className="wide"><StoreCopy value={{ en: 'Additional Instructions', ar: 'تعليمات إضافية' }} inline /><textarea name="instructions" rows={3} defaultValue={formSnapshot.instructions ?? ''} /></label></div>}{step === 3 && <DeliverySlotSelector />}{step === 4 && <StoreState kind="unavailable" title={{ en: 'Payment provider not configured', ar: 'موفر الدفع غير مهيأ' }} description={{ en: 'No payment option is rendered until a verified provider is connected. Raw card data is never stored here.', ar: 'لن يظهر خيار دفع حتى يتم ربط موفر موثق، ولا تُخزن بيانات البطاقة الخام هنا.' }} />}{step === 5 && <div className="store-review"><StoreState kind="unavailable" title={{ en: 'Order submission unavailable', ar: 'إرسال الطلب غير متاح' }} description={{ en: 'Address, shipping, tax and payment configuration must be verified before a real order can be submitted.', ar: 'يجب التحقق من العنوان والشحن والضريبة والدفع قبل إرسال طلب حقيقي.' }} />{cart.map((line) => <div key={cartLineKey(line)}><ShoppingBag /><StoreCopy value={line.product.name} /><span>× {line.quantity}</span><ProductPrice product={line.product} size="s" /></div>)}</div>}{order.status === 'placed' && <div className="store-inline-success" role="status"><CheckCircle2 /><StoreCopy value={{ en: `Order received${order.orderNumber ? ` · ${order.orderNumber}` : ''}. Totals were computed by the server; payment follows after provider activation.`, ar: `تم استلام الطلب${order.orderNumber ? ` · ${order.orderNumber}` : ''}. تم احتساب الإجماليات عبر الخادم؛ والدفع لاحقًا بعد تفعيل الموفر.` }} inline /></div>}{order.status === 'auth-required' && <div className="store-inline-error" role="alert"><X /><StoreCopy value={{ en: 'Sign in to your store account to place this order.', ar: 'سجل الدخول إلى حساب متجرك لإتمام هذا الطلب.' }} inline /><Link className="store-button store-button-secondary" to="/store/login"><StoreCopy value={{ en: 'Sign in', ar: 'تسجيل الدخول' }} inline /></Link></div>}{order.status === 'failed' && <div className="store-inline-error" role="alert"><X /><StoreCopy value={{ en: `Order was not submitted (${order.message}). No amount was charged.`, ar: `لم يتم إرسال الطلب (${order.message}). لم يتم تحصيل أي مبلغ.` }} inline /></div>}<footer>{step > 1 && order.status !== 'placed' && <button type="button" className="store-button store-button-secondary" onClick={() => setStep(step - 1)}><StoreCopy value={{ en: 'Back', ar: 'السابق' }} inline /></button>}<button type="submit" className="store-button store-button-primary" disabled={order.status === 'placing' || order.status === 'placed'}>{step === 5 ? <StoreCopy value={{ en: order.status === 'placing' ? 'Placing order…' : order.status === 'placed' ? 'Order placed' : 'Place Order', ar: order.status === 'placing' ? 'جارٍ إرسال الطلب…' : order.status === 'placed' ? 'تم إرسال الطلب' : 'تأكيد الطلب' }} inline /> : <StoreCopy value={{ en: 'Continue', ar: 'متابعة' }} inline />}<DirectionArrow /></button></footer></form><CartSummary checkout /></div></div>;
}
