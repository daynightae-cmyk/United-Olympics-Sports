...
export function ProductPrice({ product, size = 'm' }: { product: StoreProduct; size?: 's' | 'm' | 'l' }) {
  ...
  return (
    <span className="store-price-group">
      <strong className={`store-price store-price-${size}`}>{format(product.price)}</strong>
      {product.compareAtPrice != null && (
        <del aria-label={locale === 'ar' ? 'السعر السابق' : 'Previous price'}>
...
export function ProductMedia({
  ...
  useEffect(() => {
    if (imageRef.current?.complete && imageRef.current.naturalWidth > 0) setLoaded(source);
  }, [source]);
  ...
  return (
    <div
      className={`store-product-media ${hero ? 'is-hero' : ''} ${
        showImage && loaded !== source ? 'is-loading' : ''
      }`}
      aria-busy={Boolean(showImage && loaded !== source)}
    >
      {showImage ? (
        <img
          ref={imageRef}
        ...
        <div
          className="store-media-fallback"
          role="img"
          aria-label={`Image unavailable: ${product.name.en} | الصورة غير متاحة`}
        >
          <ShoppingBag aria-hidden="true" />
          <StoreCopy value={{ en: 'Image unavailable', ar: 'الصورة غير متاحة' }} />
        </div>
      )}
    </div>
  );
...
export function ProductFacts({ product }: { product: StoreProduct }) {
  ...
  const labels = {
    ...
    preorder: { en: 'Preorder', ar: 'طلب مسبق' },
  };
  return (
    <>
      {product.availability && (
        <span className="store-availability">
          <StoreCopy value={labels[product.availability]} inline />
        </span>
      )}
...
export function ProductVariants({
  ...
  const { locale } = useStore();
  return (
    <div className={`store-variant-picker ${compact ? 'is-compact' : ''}`}>
      {!!product.colors?.length && (
        <fieldset>
          <legend>
            {locale === 'ar' ? 'اللون' : 'Color'}
            {color && <span> · {product.colors.find((item) => item.en === color)?.[locale]}</span>}
          </legend>
          <div className="store-color-options">
            {product.colors.map((item) => (
              <button
                key={`color-${product.id}-${item.en}`}
                type="button"
                aria-label={`${item.en} | ${item.ar}`}
                title={item[locale]}
                aria-pressed={color === item.en}
                onClick={() => onColor(item.en)}
                className={color === item.en ? 'is-selected' : ''}
              >
                {product.colorSwatches?.[item.en] ? (
                  <span
                    className="store-color-dot"
                    style={{ '--swatch': product.colorSwatches[item.en] } as CSSProperties}
                  >
                    {color === item.en && <Check aria-hidden="true" />}
                  </span>
                ) : (
                  <span>
                    {item[locale]}
      ...
      {!!product.sizes?.length && (
        <fieldset>
          <legend>{locale === 'ar' ? 'المقاس' : 'Size'}</legend>
          <div className="store-size-options">
            {product.sizes.map((item) => (
              <button
                type="button"
                key={`size-${product.id}-${item}`}
...
export function ProductCard({ product, compact = false }: { product: StoreProduct; compact?: boolean }) {
  ...
  const handleQuickAdd = () => {
    ...
    const selectedColor = color || product.colors?.[0]?.en;
    addToCart(product, { size: selectedSize, color: selectedColor, openCart: true });
    setAdded(true);
  };

  return (
    <article className={`store-product-card ${compact ? 'is-compact' : ''}`}>
      <div className="store-product-card-stage">
        <Link to={`/store/product/${product.slug}`} aria-label={`${product.name.en} | ${product.name.ar}`}>
          <ProductMedia product={product} source={source} />
        </Link>
      ...
      <div className="store-product-card-copy">
        ...
        <span className="sr-only" role="status">
          {added
            ? `${product.name[locale]} ${locale === 'ar' ? 'تمت إضافته إلى السلة' : 'added to cart'}`
            : ''}
        </span>
      </div>
    </article>
