...
export function ProductInformation({ product }: { product: StoreProduct }) {
  ...
  return <section className="store-product-tabs"><div role="tablist" aria-label="Product details | تفاصيل المنتج">{tabs.map((label, index) => <button type="button" role="tab" id={`${prefix}-tab-${index}`} aria-controls={`${prefix}-panel`} aria-selected={active === index} tabIndex={active === index ? 0 : -1} className={active === index ? 'is-active' : ''} key={label.en} onClick={() => setActive(index)} onKeyDown={(event) => { const rtl = locale === 'ar'; const nextIndex = event.key === 'ArrowRight' ? (index + (rtl ? -1 : 1) + tabs.length) % tabs.length : event.key === 'ArrowLeft' ? (index + (rtl ? 1 : -1) + tabs.length) % tabs.length : event.key === 'Home' ? 0 : event.key === 'End' ? tabs.length - 1 : null; if (nextIndex === null) return; event.preventDefault(); setActive(nextIndex); document.getElementById(`${prefix}-tab-${nextIndex}`)?.focus(); }}>{label[locale]}</button>)}</div>
