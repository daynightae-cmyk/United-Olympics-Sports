...
const enrichedSessions: SessionWithDetails[] = demoSessions
  ...
  .map((session) => {
    ...
    return {
      ...
      childName: child ? { en: child.nameEn, ar: child.nameAr } : { en: 'Unknown', ar: 'غير معروف' },
    };
  });
...
export function ParentPortalSchedulePage() {
  ...
  const filteredSessions = selectedChild === 'all'
    ...
    : enrichedSessions.filter(s => {
        ...
        return child?.groupId === s.groupId;
      });
  ...
  return (
    <div className="admin-page">
      <PageHeader
        ...
        description={bi('Family training schedule preview — no live sessions.', 'جدول تدريبي عائلي تجريبي — لا توجد حصص مباشرة.')}
        actions={
      ...
      <section className="schedule-toolbar" aria-label="Schedule navigation">
        ...
        <div className="child-selector">
          <label>
            <BilingualText value={bi('Child', 'الطفل')} />
                      ...
                      sessions.map((session) => (
                        <article key={session.id} className="schedule-session-card">
                          ...
                          <div className="session-meta">
                            <span><Users size={12} /><BilingualText value={session.childName} /></span>
          ...
          <div className="schedule-list">
                ...
                <article key={session.id} className="schedule-session-card list-card">
                  ...
                  <div className="list-session-main">
                    ...
                    <div className="list-session-details">
                      ...
                      <span><Users size={14} /><BilingualText value={session.childName} /></span>
            ...
            {filteredSessions.length === 0 && (
              <div className="admin-preview-card" style={{ padding: 32, textAlign: 'center' }}>
                ...
                <p><BilingualText value={bi('Preview schedule is empty for selected child. Real sessions will appear when connected to backend.', 'الجدول التجريبي فارغ للطفل المحدد. الحصص الحقيقية ستظهر عند الاتصال بالخادم.')} /></p>
              </div>
            )}
          </div>
        </section>
