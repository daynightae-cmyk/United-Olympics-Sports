...
import { EnterpriseEmpty, EnterpriseKpi, EnterpriseSelect, EnterpriseStatus, EnterpriseTable, EnterpriseToolbar, ExportMenu, PreviewNotice } from '../../components/enterprise/EnterpriseUI';
...
export function AdminPaymentsPage() {
  ...
  return <div className="admin-page">
    ...
    {loading && <div className="enterprise-panel"><UiPreviewState title={bi('Loading payments', 'جارٍ تحميل المدفوعات')} description={bi('Preparing the preview transaction ledger.', 'جارٍ تجهيز دفتر المعاملات التجريبي.')} /></div>}
    {error && <div className="enterprise-panel"><UiPreviewState title={bi('Payment ledger unavailable', 'تعذر عرض دفتر المدفوعات')} description={bi('The Admin gateway returned an error.', 'أعادت بوابة الإدارة خطأً.')} /></div>}
    {!loading && !error && (payments.length ? <><EnterpriseTable caption={bi('Payment transaction workspace', 'مساحة معاملات المدفوعات')}><thead><tr>{[bi('Payment', 'الدفعة'), bi('Player', 'اللاعب'), bi('Subscription', 'الاشتراك'), bi('Amount', 'المبلغ'), bi('Method', 'الطريقة'), bi('Date', 'التاريخ'), bi('Status', 'الحالة'), bi('Actions', 'الإجراءات')].map((label) => <th key={label.en}><BilingualText value={label} /></th>)}</tr></thead><tbody>{payments.map((payment) => <tr key={payment.id}><td><strong><Link to={`/admin/payments/${payment.id}`}>{payment.id}</Link></strong><small>{payment.reference ?? bi('No reference', 'لا يوجد مرجع').en}</small></td><td><BilingualText value={playerName(payment.playerId)} /></td><td><Link to={`/admin/subscriptions/${payment.subscriptionId}`}>{payment.subscriptionId}</Link></td><td><strong>{payment.amount} {payment.currency}</strong></td><td><BilingualText value={payment.method} /></td><td>{payment.paidAt}</td><td><EnterpriseStatus label={paymentStatusLabel(payment.status)} tone={paymentTone(payment.status)} /></td><td><Link className="admin-link-button" to={`/admin/payments/${payment.id}`}><BilingualText value={bi('Open payment', 'فتح الدفعة')} /></Link></td></tr>)}</tbody></EnterpriseTable><section className="finance-mobile-cards">{payments.map((payment) => <article className="enterprise-panel" key={payment.id}><div className="mobile-finance-head"><div><strong><Link to={`/admin/payments/${payment.id}`}>{payment.id}</Link></strong><small><BilingualText value={playerName(payment.playerId)} /></small></div><EnterpriseStatus label={paymentStatusLabel(payment.status)} tone={paymentTone(payment.status)} /></div><p>{payment.amount} {payment.currency} · <BilingualText value={payment.method} /></p><Link className="admin-link-button" to={`/admin/payments/${payment.id}`}><BilingualText value={bi('Open payment', 'فتح الدفعة')} /></Link></article>)}</section></> : <EnterpriseEmpty title={bi('No payments match', 'لا تطابق أي مدفوعات')} description={bi('Try a different status, method or search term.', 'جرب حالة أو طريقة أو مصطلح بحث آخر.')} />)}
  </div>;
}
