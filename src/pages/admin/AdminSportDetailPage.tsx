...
export function AdminSportDetailPage() {
  ...
  const deleteSport = async () => {
    if (!isPreview || hasRelations) return;
  ...
  return <div className="admin-page">
    ...
    <section className="admin-panel"><div className="panel-heading"><BilingualText value={bi('Sport Controls', 'ضوابط الرياضة')} /><ShieldCheck /></div><div className="preview-form-grid"><label><BilingualText value={bi('Operating status', 'حالة التشغيل')} /><select value={sport.status} disabled={updating || !isPreview} onChange={event => void setStatus(event.target.value as 'active' | 'inactive')}><option value="active">Active | نشط</option><option value="inactive">Inactive | غير نشط</option></select></label></div>{isPreview ? <p className="preview-warning"><BilingualText value={bi('Changes persist in the browser Preview store.', 'تستمر التغييرات في مخزن المعاينة بالمتصفح.')} /></p> : <p className="enterprise-result"><BilingualText value={bi('The Olympic sport catalog is migration-managed and read-only in production.', 'كتالوج الرياضات الأولمبية مُدار بالترحيلات وهو للقراءة فقط في الإنتاج.')} /></p>}</section>
    ...
    <section className="admin-panel"><div className="panel-heading"><BilingualText value={bi('Danger Zone', 'منطقة الخطر')} /><ShieldCheck /></div>{!isPreview && <p className="enterprise-result"><BilingualText value={bi('Sport deletion is disabled in production to preserve relational integrity.', 'حذف الرياضات معطل في الإنتاج للحفاظ على سلامة العلاقات.')} /></p>}{isPreview && hasRelations && <p className="preview-warning"><BilingualText value={bi('Remove or reassign related programs, groups, players and coaches before deleting this sport.', 'احذف أو أعد تعيين البرامج والمجموعات واللاعبين والمدربين المرتبطين قبل حذف هذه الرياضة.')} /></p>}<UiButton variant="danger" disabled={deleting || hasRelations || !isPreview} onClick={() => void deleteSport()}><Trash2 /><BilingualText value={bi(deleting ? 'Deleting…' : isPreview ? 'Delete Preview sport' : 'Delete sport', deleting ? 'جارٍ الحذف…' : isPreview ? 'حذف رياضة المعاينة' : 'حذف الرياضة')} /></UiButton></section>
  </div>;
}
