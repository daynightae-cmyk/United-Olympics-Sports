...
export function AdminReportsPage() {
  ...
  return <div className="admin-page">
    <PageHeader icon={FileBarChart} eyebrow={bi('Insights & Governance', 'الرؤى والحوكمة')} title={bi('Reports Studio', 'استوديو التقارير')} description={isPreview ? bi('Generate and review persistent preview report records from the current Admin data provider.', 'أنشئ وراجع سجلات تقارير معاينة مستمرة من موفر بيانات الإدارة الحالي.') : bi('Live scope aggregates with CSV export. Generated report records are a deferred engine.', 'مجمعات النطاق الحية مع تصدير CSV. سجلات التقارير المنشأة محرك مؤجل.')} actions={<div className="admin-header-actions">{isPreview && <PreviewNotice />}<ExportMenu filename={isPreview ? 'organization-report-preview.csv' : 'organization-report-live.csv'} headers={['Sport','Programs','Groups','Players','Sessions']} rows={rows} /></div>} />
    {isPreview && notice && <div className="preview-warning" role="status"><BilingualText value={bi('A report record was generated in browser Preview data.', 'تم إنشاء سجل تقرير في بيانات المعاينة بالمتصفح.')} /></div>}
    ...
    <section className="enterprise-grid-2" style={{ marginTop: 14 }}>{isPreview
      ? <EnterprisePanel title={selectedReport.label} description={selectedReport.detail}><p><BilingualText value={bi('Generation creates a report record only; it does not claim a production analytics pipeline or external file delivery.', 'الإنشاء ينشئ سجل تقرير فقط؛ ولا يدعي وجود مسار تحليلات إنتاجي أو تسليم ملف خارجي.')} /></p><button type="button" className="admin-primary-button" disabled={generateLoading} onClick={() => void generateSelected()}><Plus size={15} /><BilingualText value={generateLoading ? bi('Generating…', 'جارٍ الإنشاء…') : bi('Generate Preview Report', 'إنشاء تقرير معاينة')} /></button></EnterprisePanel>
      : <EnterprisePanel title={selectedReport.label} description={selectedReport.detail}><p><BilingualText value={bi('Live aggregates above and CSV export are computed from real gateway data. A generated-report records engine is deferred.', 'المجمعات الحية أعلاه وتصدير CSV محسوبان من بيانات البوابة الحقيقية. محرك سجلات التقارير المنشأة مؤجل.')} /></p></EnterprisePanel>}<EnterprisePanel title={bi('Current Data Scope', 'نطاق البيانات الحالي')} description={bi('Counts read directly from the Admin gateway.', 'أعداد تُقرأ مباشرة من بوابة الإدارة.')}><div className="enterprise-kpi-grid"><EnterpriseKpi label={bi('Programs', 'البرامج')} value={programs.length} icon={Flag} /><EnterpriseKpi label={bi('Groups', 'المجموعات')} value={groups.length} icon={UsersRound} /><EnterpriseKpi label={bi('Players', 'اللاعبون')} value={players.length} icon={UsersRound} /><EnterpriseKpi label={bi('Sessions', 'الجلسات')} value={sessions.length} icon={CalendarDays} /></div></EnterprisePanel></section>
    ...
    {isPreview && <EnterprisePanel title={bi('Generated Report Records', 'سجلات التقارير المنشأة')} description={bi('Persistent Preview records generated by this workspace.', 'سجلات معاينة مستمرة أنشأتها هذه المساحة.')}>
