...
const portalMeta: Record<
  PortalKind,
  { title: BilingualValue; role: BilingualValue; nav: PortalNavItem[] }
> = {
  player: {
    title: bi("Player Portal", "بوابة اللاعب"),
    role: bi("Athlete Workspace", "مساحة اللاعب"),
    nav: [
  ...
  parent: {
    ...
    role: bi("Family Workspace", "مساحة الأسرة"),
    nav: [
      { path: "", label: bi("Overview", "نظرة عامة"), icon: Home },
  ...
  coach: {
    ...
    role: bi("Training Workspace", "مساحة التدريب"),
    nav: [
      { path: "", label: bi("Overview", "نظرة عامة"), icon: Home },
      { path: "schedule", label: bi("Schedule", "الجدول"), icon: CalendarDays },
    ...
    <div className={`portal-shell portal-${portal}`}>
      ...
      >
        ...
        <div className="portal-role">
          <small>
            <BilingualText value={bi("Preview Product", "منتج تجريبي")} />
          </small>
          <BilingualText value={meta.title} />
          <span>
            <BilingualText value={meta.role} />
          </span>
        </div>
        <nav className="portal-nav">
          {meta.nav.map(({ path, label, icon: Icon }) => (
            <NavLink key={path || "overview"} to={path ? `${base}/${path}` : base} end={!path}>
              <Icon />
        ...
        <Link className="portal-public-link" to="/">
          <ChevronLeft />
      ...
      <section className="portal-workspace">
        <header className="portal-topbar">
          ...
          <div>
            ...
            <strong>
              <BilingualText value={current.label} />
            </strong>
          </div>
          <span className="portal-preview-badge">
            <span />
