...
const routeLabels: Record<string, { en: string; ar: string }> = {
  sports: bi("Sports", "الرياضات"),
  players: bi("Players", "اللاعبون"),
  groups: bi("Training Groups", "مجموعات التدريب"),
  parents: bi("Parents", "أولياء الأمور"),
  coaches: bi("Coaches", "المدربون"),
  programs: bi("Programs", "البرامج"),
  schedules: bi("Schedules", "الجداول"),
  attendance: bi("Attendance", "الحضور"),
  performance: bi("Performance", "الأداء"),
  countries: bi("Countries", "الدول"),
  branches: bi("Branches", "الفروع"),
  subscriptions: bi("Subscriptions", "الاشتراكات"),
  payments: bi("Payments", "المدفوعات"),
  reports: bi("Reports", "التقارير"),
  content: bi("Content", "المحتوى"),
  users: bi("Users & Roles", "المستخدمون والصلاحيات"),
  settings: bi("Settings", "الإعدادات"),
};
...
function usePageTitle() {
  ...
  return useMemo(() => {
    ...
    const player = getPlayer(last);
    if (player) return { en: player.nameEn, ar: player.nameAr };
    const sport = getSport(last);
    if (sport) return sport.name;
    const group = getGroup(last);
...
export function AdminLayout() {
  ...
  useEffect(() => {
    setCollapsed(sidebarDefault === "collapsed");
  }, [sidebarDefault]);

  return (
    <div
      className={`admin-shell ${collapsed ? "sidebar-collapsed" : ""} density-${density} font-${fontScale}`}
    >
      <AdminSidebar
        open={sidebarOpen}
        collapsed={collapsed}
        ...
        onCollapse={() => {
          ...
          setSetting("sidebarDefault", nextCollapsed ? "collapsed" : "expanded");
        }}
      />
      {sidebarOpen && (
        <button
          ...
          aria-label="Close navigation | إغلاق القائمة"
        />
      )}
        ...
        <main className="admin-main">
          ...
          <Routes>
            ...
            {Object.keys(routeLabels)
              .filter((path) => !["sports", "players", "settings"].includes(path))
              .map((path) => (
                <Route key={path} path={path} element={<AdminFutureModulePage />} />
              ))}
