...
=== AUTH (AuthRouter) ===
Route: /auth — AuthLoginPage (new) — mobile: requires CSS — RTL: requires review
Route: /auth/login — same page
Route: /auth/register — AuthRegisterPage (new) — mobile: requires CSS
Route: /auth/reset — AuthResetPage (new) — mobile: requires CSS
Note: No duplicate existing auth directory under src/portals/player/auth — AuthRouter does NOT conflict

=== PLAYER PORTAL ===
Layout: PortalLayout (portal="player")
Route: /player (index) — PlayerPortalOverviewPage — Black Gold identity: must verify
Route: /player/schedule — PlayerPortalSchedulePage
...
Route: /player/settings? — Not explicitly routed; must verify if reachable
Route: /player/404 — not explicitly defined; relies on router fallback? Must inspect
